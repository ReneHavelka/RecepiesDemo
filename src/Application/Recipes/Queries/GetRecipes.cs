...
namespace Application.Recipes.Queries
{
	...
	public class GetRecipes
	{
		...
		public async Task<IList<Recipe>> GetRecipeIdsDishTypesIdsNamesListAsync(int dishTypeId)
		{
			...
			if (dishTypeId == -1)
			{
				var recipesQry = from recipe in _dbContext.RecipesDemo
								 join dishType in _dbContext.DishTypesDemo
			...
			return await _dbContext.RecipesDemo.Where(x => x.DishTypeId == dishTypeId)
		...
		public async Task<IList<Recipe>> GetCompleteRecipes(IEnumerable<int> dishTypeIds = null)
		{
			if (dishTypeIds != null)
			{
				return await _dbContext.RecipesDemo.Where(x => dishTypeIds.Contains(x.DishTypeId)).ToListAsync();
			}

			return await _dbContext.RecipesDemo.ToListAsync();
		}
	}
}
